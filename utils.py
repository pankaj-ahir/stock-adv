...
#     # सुनिश्चित करें कि यह scalar values है
#     avg_volume = df['Volume'].rolling(window=20).mean().iloc[-1]
#     current_volume = df['Volume'].iloc[-1]

#     # अगर ये Series हैं तो scalar में कन्वर्ट करें
#     if isinstance(avg_volume, pd.Series) or isinstance(avg_volume, pd.DataFrame):
#         avg_volume = avg_volume.values[0] if len(avg_volume) > 0 else float('nan')

#     if isinstance(current_volume, pd.Series) or isinstance(current_volume, pd.DataFrame):
#         current_volume = current_volume.values[0] if len(current_volume) > 0 else float('nan')

#     # अब scalar values के लिए NaN चेक करें
